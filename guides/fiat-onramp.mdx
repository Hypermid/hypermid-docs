...
console.log("Payment methods:", config.paymentModes);
// ["card", "debit_card", "bank_transfer", ...]
```
...
for (const asset of assets.assets) {
  console.log(`${asset.symbol} on chain ${asset.cryptoChain}: min ${asset.minAmount}, max ${asset.maxAmount}`);
}
```
