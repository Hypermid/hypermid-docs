...
console.log("Send to:", exec.depositAddress);
console.log("Memo:", exec.memo);         // Required for XRP
console.log("Deposit ID:", exec.depositId);

// Track with deposit status endpoint
const status = await client.getDepositStatus({
  depositId: exec.depositId,
});
```
